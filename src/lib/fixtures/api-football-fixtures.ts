...
export async function fetchWorldCupApiFixtures(): Promise<ApiFootballFixture[]> {
  ...
  const [livePayload, lastPayload, nextPayload, windowPayload] = await Promise.all([
    fetchApiFootball<ApiFootballFixture[]>("/fixtures", { live: "all" }),
    fetchApiFootball<ApiFootballFixture[]>("/fixtures", { ...wc, last: "30" }),
  ...
  return mergeApiFootballFixturesById([
