...
import { notifyUserPush, sendWebPushToUser } from "@/lib/push/send";
...
export function notifyDailyMatchesPreview(userId: string, matchCount: number) {
  if (matchCount < 1) return;
  notifyUserPush(userId, {
    title: "Matches on today",
    body: `${matchCount} World Cup fixture${matchCount === 1 ? "" : "s"} on the schedule.`,
    url: "/#tournament",
    tag: `matches:preview:${utcDateKey()}`
  });
}
